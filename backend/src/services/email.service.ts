...
export class EmailService {
  ...
  async sendEmail(to: string, subject: string, html: string): Promise<void> {
    try {
      ...
      logger.error(`EmailService: Failed to send email to ${to}: ${errorMessage}`);
   ...
   */
  async sendWeeklyDigest(subscribers: string[], articles: ArticleSummary[]): Promise<void> {
    if (subscribers.length === 0) {
      logger.info('EmailService: No subscribers for weekly digest, skipping');
      return;
    }

    if (articles.length === 0) {
      logger.info('EmailService: No new articles for weekly digest, skipping');
      return;
    }
    ...
    const results = await Promise.allSettled(
      subscribers.map((subscriberEmail) =>
    ...
    logger.info(
      `EmailService: Weekly digest sent. Success: ${succeeded}, Failed: ${failed}, Total: ${subscribers.length}`
    );
    ...
    if (failed > 0) {
      ...
      failures.forEach(({ result, email }) => {
        ...
        logger.warn(`EmailService: Failed to send digest to ${email}: ${reason}`);
      });
    }
  }
  ...
  async sendPasswordReset(to: string, resetToken: string): Promise<void> {
    ...
    await this.sendEmail(
      to,
      'PharmaConnect — Password Reset Request',
      `
   ...
   */
  async sendComplianceAlertEmail(
    to: string,
    itemName: string,
